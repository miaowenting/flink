...
public abstract class PatternProcessFunction<IN, OUT> extends AbstractRichFunction {
	...
	//	--------------------
	private Boolean flagNeedListener = false;

	private CepListener<IN> listener = null;

	public Boolean getFlagNeedListener() {
		return flagNeedListener;
	}

	/**
	 * 逻辑注入 cep 规则的监听器
	 */
	public void registerListener(CepListener<IN> listener) {
		flagNeedListener = true;
		this.listener = listener;
	}

	/**
	 * 根据逻辑注入的 cep 规则，构造新的 cep pattern
	 */
	public Pattern getNewPattern(IN flagElement) {
		return listener.returnPattern(flagElement);
	}

	public Boolean needChange(IN element) {
		return listener.needChange(element);
	}
    //	----------------------
