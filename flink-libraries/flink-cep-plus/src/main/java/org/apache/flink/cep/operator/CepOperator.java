...
public class CepOperator<IN, KEY, OUT>
	...
	private void changeNFA(IN flagElement) throws Exception {
		...
		// 从外部获取新的 pattern
		Pattern newPattern = userFunction.getNewPattern(flagElement);

		// 构建新的 NFA
		NFACompiler.NFAFactoryCompiler<IN> nfaFactoryCompiler = new NFACompiler.NFAFactoryCompiler<>((Pattern<IN, ?>) newPattern);
		nfaFactoryCompiler.compileFactory();
		boolean timeoutHandling = userFunction instanceof TimedOutPartialMatchHandler;
		NFACompiler.NFAFactory nfaFactory = NFACompiler.compileFactory(newPattern, timeoutHandling);
		NFA<IN> newNFA = nfaFactory.createNFA();

		// 这个地方为所有的边 transition 设置了 cepRuntimeContext
